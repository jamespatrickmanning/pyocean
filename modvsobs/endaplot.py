...
dfeobs=pd.read_csv(input_dir_e+'BF01botttemp_da_obs.csv',sep=',',parse_dates={'datet':[0,1,2]},index_col='datet',date_parser=parse,names=['yy','mm','dd','count','mean','median','min','max','std','rms'])
...
dfnobs=pd.read_csv(input_dir_n+'I01botttemp_da_obs.csv',sep=',',parse_dates={'datet':[0,1,2]},index_col='datet',date_parser=parse,names=['yy','mm','dd','count','mean','median','min','max','std','rms'])
...
dfeom=pd.read_csv(input_dir_e+'BF01botttemp_da_mod_da_obs.csv',sep=',',skiprows=1,parse_dates=True,index_col=[0],names=['date','mean','median','min','max','std','rms'])
dfnom=pd.read_csv(input_dir_n+'I01botttemp_da_mod_da_obs.csv',sep=',',skiprows=1,parse_dates=True,index_col=[0],names=['date','mean','median','min','max','std','rms'])
...
ax1.plot(dfeobs.index,dfeobs['mean'].values,'r-')
ax1.plot(nafter1.index,nafter1['mean'].values,'b-')
...
ax2.plot(dfeom.index,dfeom['mean'].values,'r-')
ax2.plot(nafter2.index,nafter2['mean'].values,'b-')
ax2.set_ylabel('temperature difference')
ax2.set_title('I01 VS BF01 bottom observed-modeled temperature')
ax2.grid(True)
##########BN01 site#################
plt.show()
plt.savefig('I01VSBF01.png')
