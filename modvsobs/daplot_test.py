...
def parse(datet):
        ...
        return dt
def season_cycle(data):
    newindex=[]
    for i in range(len(data)):
        newindex.append(data['mean'].index[i].replace(year=2000))
    newindex1=set(newindex)
    Data=pd.DataFrame(range(len(newindex1)),index=newindex1)
    Temp=[]
    for i in Data.index:
        temp=[]
        for j in data.index:
            if i.month==j.month and i.day==j.day:
                if data['mean'][j]>0:
                    temp.append(round(data['mean'][j],2))
        Temp.append(temp)
    mean=[]
    for i in range(len(Temp)):
        a=np.mean(np.array(Temp[i]))
        mean.append(round(a,2))
    Data['mean']=pd.Series(mean,index=Data.index)
...
def remove_seacycle(df,DF):
    ...
    for j in range(len(df)):
        for i in range(len(DF)):
            if DF.index[i].month==df.index[j].month and DF.index[i].day==df.index[j].day:
                if DF['mean'][i]>0:
                    q=df['mean'][j]-DF['mean'][i]
...
if w_fig==61 or w_fig==62:
    ...
    DF1m=season_cycle(df1m)
    df1o=remove_seacycle(df1o,DF1o)
...
if w_fig==61 or w_fig==62:
  ...
  DF=season_cycle(df)
  DF00=season_cycle(df00)
  
  df=remove_seacycle(df,DF)
  df00=remove_seacycle(df00,DF00)
  ...
  if site=='AG01':
      ...
      aorb='b'
  plt.show()
  plt.savefig('/net/nwebserver/epd/ocean/MainPage/modvsobs/figs/fig6'+aorb+'_'+site+'_'+var+'_anomaly.png')
  plt.savefig('/net/nwebserver/epd/ocean/MainPage/modvsobs/figs/fig6'+aorb+'_'+site+'_'+var+'_anomaly.eps')
  '''
  ax=fig.add_subplot(212,sharex=ax1,sharey=ax1)
  ax.plot(DF00.index,DF00['mean'].values,color=color1) 
  ...
  ax.tick_params(axis='both', which='major', labelsize=20)
  plt.setp(ax1.get_xticklabels(), visible=False)
  dates = mpl.dates.drange(min(DF.index), max(DF.index), timedelta(days=31))
